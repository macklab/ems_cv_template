...
# Probably need to format this differently for each section (esp. academic articles; could i just put the DOI?)
...
print_section <- function(cv, section_id, glue_template = "default"){
  ...
  if(section_id == "awards"){
    glue_template <- "
{title} \\hfill {timeline}
\n\n\n"  
  }
  else if(section_id == "presentations_talks" | section_id == "presentations_posters" | section_id == "presentations_other"){
    glue_template <- "
{title}  
*{institution}, {loc}*
\n\n\n"  
  }
  else if(section_id == "publications"){
